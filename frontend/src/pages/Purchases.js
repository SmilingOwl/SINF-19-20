  ...
  useEffect(() => {
    const updateAccountsPayable = async () => {
      ...
      if(balanceSheet.non_current_assets) {
        let non_current = balanceSheet.non_current_assets.filter(p => p.index === 'A00144');
        if(non_current.length > 0) {
          accounts_payable += non_current[0].value;
        }
        let current = balanceSheet.current_assets.filter(p => p.index === 'A00150');
        if(current.length > 0) {
          ...
          <div className="row mtop-smaller">
            ...
            <div className="col-md-3 smallBox align-items-center d-flex">
              <div className="col-md-12 centered">
                ...
                <p className="price" style={{'text-align':'center'}}>{numberWithSpaces(totalSpent.toFixed(2))} €</p>
              </div>
            </div>
            <div className="col-md-2"/>
            <div className="col-md-3 smallBox align-items-center d-flex">              
              <div className="col-md-12 centered">
                ...
                <p className="price" style={{'text-align':'center'}}>{ numberWithSpaces(accountsPayable.toFixed(2))} €</p>
              </div>
            </div>
          </div>
        );
  ...
  for (let i = 1; i <= suppliers.length && i <= 10 ; i++) {
    suppliersTable.push(
      <tr className="table-row" key={i}>
        ...
        <td className="centered">{numberWithSpaces(suppliers[i-1].total_spent.toFixed(2))} €</td>
      </tr>
    );
  }
  ...
  for (let i = 1; i <= products.length && i <= 10 ; i++) {
    productsTable.push(
      <tr className="table-row" key={i}>
        ...
        <td className="centered">{numberWithSpaces((products[i-1].totalSpent / products[i-1].unitsSold).toFixed(2)) } € </td>
        <td className="centered">{numberWithSpaces(products[i-1].totalSpent.toFixed(2))} €</td>
      </tr>
    );
